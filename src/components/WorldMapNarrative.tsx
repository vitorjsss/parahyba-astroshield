...
interface WorldMapNarrativeProps {
...
export function WorldMapNarrative({ onMapClick, impactPoint, selectedAsteroid, impactResults, onClearSimulation }: WorldMapNarrativeProps) {
    ...
    useEffect(() => {
        ...
        if (impactResults && impactResults.crater_diameter_km && impactResults.energy_megatons_tnt &&
            ...
            if (svgSelectionRef.current && zoomBehaviorRef.current) {
                ...
                // Calcular o zoom necessário para que toda a área caiba na tela com margem
                const { width, height } = dimensions;
                const margin = 80; // Margem otimizada
                const availableSpace = Math.min(width, height) / 2 - margin;

                // Calcular o fator de zoom baseado no raio mais externo
                const currentTransform = d3.zoomTransform(svgSelectionRef.current.node()!);
                const targetScale = Math.min(availableSpace / maxRadiusPx, 4); // Aumento do limite máximo para 4x
                const finalScale = Math.max(targetScale * 0.85, 1.5); // Zoom mínimo aumentado para 1.5x

                console.log("🔍 ZOOM AUTOMÁTICO MELHORADO:", {
                    impactCoords: coords,
                    maxRadiusPx,
                    availableSpace,
                    targetScale,
        ...
        // Fallback simples se não há dados completos - COM ZOOM
        else {
            ...
            // 🔍 ZOOM AUTOMÁTICO PARA IMPACTO SIMPLES
            if (svgSelectionRef.current && zoomBehaviorRef.current) {
                const { width, height } = dimensions;

                // Zoom padrão mais próximo para impactos simples
                const defaultZoomScale = 3.5;

                console.log("🔍 ZOOM SIMPLES:", {
                    impactCoords: coords,
